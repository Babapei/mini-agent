...
八个阶段都完成后，更新 `docs/DESIGN.md` 的「最大限制」，把已经落地的能力从限制里移出。README 的项目结构补上新工具和开关。真实模型仍由人看 Trace，不把 Mock 的固定句子套上去。E8 完成时 `pytest` 是 44 项。后来补上「恢复提示排在同一轮全部工具结果之后」的测试，当前全量是 45 项通过。
