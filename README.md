...
一个用 Python 标准库实现的命令行 Agent。它接收一句自然语言任务，由模型决定调用工具还是给出最终答案；程序执行工具后把结果交回模型，再决定下一步，直到完成或确认无法继续。进程随后退出，不做多轮对话。模型接口不保存会话，每一轮都由本进程把已有消息重新发送。需要工具时，模型通过 Function Calling 返回工具名和参数，程序在本地执行。基础工具是 `read_file`、`write_file`、`search_text` 和 `calculator`。文件限制在 `workspace/` 内。计算器用语法树解释表达式，不用 `eval`，合计必须来自计算器的返回值。Mock 和 OpenAI 兼容接口走同一个主循环。真实模型用过 DeepSeek（`deepseek-flash`）。

> 已实现的基础能力：

1. **Agent 主循环。** 每轮问模型，执行后再问下一次。
2. **Tool 定义和注册机制。** 工具按名称放在注册表里。
3. **Tool 参数解析。** 用 JSON Schema 检查参数。
4. **Tool 调用。** 按模型给出的顺序执行。
5. **Tool Result 返回 Agent。** 成功或失败都写回消息。
6. **Agent 根据 Tool Result 判断下一步行为。** 下一步仍由模型决定。
7. **支持一次任务中连续调用多个 Tool。**
8. **最终答案输出。** 没有工具调用时给出最终答案。
9. **Agent 执行轮数控制，避免无限循环。** 默认最多 12 轮，同一调用连续失败 2 次也停止。
10. **Tool 调用失败时能够进行合理处理。** 失败是一条结果，进程不崩溃。

> 已实现的增强项：

- **Tool Schema。** 每次把工具名称、说明和参数规格发给模型。
- **Tool 参数校验。** 参数不合法就不执行。
- **Tool Permission。** 可用 `--allow` 限制读、写、计算。
- **Tool 调用失败自动恢复。** 文件不存在时给一句提示，搜索仍由模型决定。
- **Retry。** 只重试网络错误和 HTTP 5xx，最多 2 次。
- **Timeout。** 请求超时 30 秒。
- **Agent Trace。** 每步写入 `trace.md` 和 `trace.jsonl`。
- **Token Usage。** 真实模型的 `usage` 写入 Trace。
- **Context Compression。** 过长时截断单条输出，并收起更早的工具结果。
- **Plan。** 第一次工具调用前可写下计划。
- **Sub Agent。** `--sub-agent` 才启用，只委托一层。
- **Streaming。** 终端可边跑边打印工具名。线上请求仍一次返回。
- **防止读取 workspace 之外的文件。** 越界路径会被拒绝。
- **Tool Search。** 按字面量查找工具说明。旧工具仍可直接调用。
- **对执行计划动态调整。** 失败后 Mock 可写计划修订。真实模型不写这一节。
